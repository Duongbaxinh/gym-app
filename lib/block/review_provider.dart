...
class ReviewProvider with ChangeNotifier{
  ...
  Future<void> addReview(List<Map<String,dynamic>> reviews) async{
    ...
    try{
      for(var review in reviews){
        CollectionReference<Map<String,dynamic>> reviewRef = fireStore.collection('review');
  ...
  Future<void> addOneReview(Map<String,dynamic> review,BuildContext context) async{
    FirebaseFirestore fireStore = FirebaseFirestore.instance;
    try{
        CollectionReference<Map<String,dynamic>> reviewRef = fireStore.collection('review');
        DocumentReference newReview = await reviewRef.add(review);
        // ignore: use_build_context_synchronously
        ShowDialog('Thank you for your contribution', 'You just wrote a comment', context);
        notifyListeners();
    }catch (err){
      throw Exception(err);
    }
  ...
  Future<List<ReviewModel>> getReviewOfTrainer(String idTrainer2) async {
    ...
    FirebaseFirestore fireStore = FirebaseFirestore.instance;
    CollectionReference<Map<String,dynamic>> reviewRef = fireStore.collection('review');
    QuerySnapshot<Map<String,dynamic>> querySnapshot =  await reviewRef.where('idTrainer', isEqualTo: idTrainer2).get();
    for(var review  in querySnapshot.docs){
      Map<String,dynamic> fullReview = review.data();
      fullReview['id'] =review.id;
      reviews.add(ReviewModel.fromJson(fullReview));
