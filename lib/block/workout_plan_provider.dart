...
class WorkOutPlanProvider with ChangeNotifier{
  ...
  Future<void> init()async {
      _isLoading  = true;
      _workPlanCategories = await getWorkoutByCategory(_currentCategory.toLowerCase());
      _isLoading = false;
  }
  ...
  Future<void> addOneWorkPlan(Map<String,dynamic> dataWorkout3) async {
    ...
    DocumentReference newWorkout = await  workPlanRef.add(dataWorkout[0]['workout']);
    for(var lesson in dataWorkout[0]['lessons'] ){
      await fireStore.collection('workPlan').doc(newWorkout.id).collection('lessons').doc().set(lesson);
    }
    await fireStore.collection('workPlan').doc(newWorkout.id).collection('trainer').doc().set(dataWorkout[0]['trainer']);
  }
  // GET WORK PLAN
  Future<TrainerModel> getTrainerByIdWorkPlan(String idWorkPlan) async{
    try{
      FirebaseFirestore fireStore = FirebaseFirestore.instance;
      CollectionReference<Map<String,dynamic>> trainerRef = fireStore.collection('workPlan').doc(idWorkPlan).collection('trainer');
      QuerySnapshot<Map<String,dynamic>> querySnapshot = await trainerRef.get();
  ...
  Future<List<LessonModel>> getLessonByWorkPlan(String id) async {
      try{
        ...
        FirebaseFirestore fireStore = FirebaseFirestore.instance;
        CollectionReference<Map<String,dynamic>> lessonRef =  fireStore.collection('workPlan').doc(id).collection('lessons');
        QuerySnapshot<Map<String,dynamic>> lessonSnapshot = await lessonRef.get();
  ...
  Future<List<WorkPlanModel>> getAllWorkOut() async{
   List<WorkPlanModel> listWorkPlanModel = [];
    try{
      FirebaseFirestore fireStore = FirebaseFirestore.instance;
      CollectionReference<Map<String,dynamic>> workOutPlan = fireStore.collection('workPlan');
      QuerySnapshot<Map<String,dynamic>> querySnapshot = await workOutPlan.get();
      for(var dataSnapshot in querySnapshot.docs){
        WorkPlanModel workPlanModel2 = WorkPlanModel.fromJson(dataSnapshot.data()!);
        List<LessonModel> lessons = await getLessonByWorkPlan(dataSnapshot.id);
        TrainerModel trainer = await getTrainerByIdWorkPlan(dataSnapshot.id);
        workPlanModel2.lessons = lessons;
        workPlanModel2.trainer = trainer;
 ...
 Future<List<WorkPlanModel>> getWorkoutByCategory(String category) async{
    ...
    try{
      FirebaseFirestore fireStore = FirebaseFirestore.instance;
      CollectionReference<Map<String,dynamic>> workOutPlan = fireStore.collection('workPlan');
      QuerySnapshot<Map<String,dynamic>> querySnapshot = await workOutPlan.where('category',isEqualTo: category).get();
      for(var dataSnapshot in querySnapshot.docs){
        WorkPlanModel workPlanModel2 = WorkPlanModel.fromJson(dataSnapshot.data()!);
        List<LessonModel> lessons = await getLessonByWorkPlan(dataSnapshot.id);
        TrainerModel trainer = await getTrainerByIdWorkPlan(dataSnapshot.id);
